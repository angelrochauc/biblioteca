...
class AdminModel extends Mysql{
    ...
    {
        $sql = "SELECT e.id, e.nombre, l.id, l.titulo, l.descripcion, p.id, p.id_estudiante, p.id_libro, p.fecha_prestamo, p.fecha_devolucion, p.cantidad, p.observacion, p.estado FROM estudiante e INNER JOIN libro l INNER JOIN prestamo p ON p.id_estudiante = e.id WHERE p.id_libro = l.id";
        $res = $this->select_all($sql);
        return $res;
    }
